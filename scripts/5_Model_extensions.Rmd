...
ggsave(here('visuals', 'sup_figure_multiple_cartels.jpg'), width = 15, height = 18, units = 'cm')
...
data = data %>%
  filter(!(x %in% c(968,3264,4974,14367,14368,17234)))%>%
  select(-values)%>%
  group_by(names) %>%
...
result <- data %>%
  pivot_wider(names_from = row, values_from = x, names_prefix = "x")

result = as.matrix(result)
result[,1] = c("5 players", "2 players", "Price setting", "Vertical monopoly", "Quantity adjustment")
values_ = result[3:5,2]
result[3:5,4] = values_
result[3:5,2] = NA

result2 = matrix(nrow = 5,ncol = 4)
result2[1,] = result[1,]
result2[2,] = result[2,]
result2[3,] = result[4,]
result2[4,] = result[3,]
...
colnames(result2) = c('Market configuration', 'Low stable steady state','Middle unstable steady state', 'High stable steady state')

print(result2)
table_to_print =  kable(result2, format = "latex", booktabs = TRUE) %>%
...
# Define equilibrium stock in case of takeover, as well as harvest
x_takeover = data_ss %>%
                   select(Takeover)%>%
                   pull()
x_cournot = data_ss %>%
...
q_takeover = growth(x_takeover)


# Define price as market clearing
price_equilibrium = function(s, market_structure = "takeover"){
  if(market_structure=="takeover"){
      y = (s*sigma^2*x_takeover^2 - W1*sigma*x_takeover)/(2*W2) - monop_takeover(x_takeover)
  }else if(market_structure == "cournot"){
      y = (s*sigma^2*x_cournot^2 - W1*sigma*x_cournot)/(2*W2) - growth(x_cournot)
  }else if(market_structure == "monopoly"){
      y = (s*sigma^2*x_monop^2 - W1*sigma*x_monop)/(2*W2) - growth(x_monop)
  }
  return(y)
}

# Find price as solving equilibrium market clearing
price_poacher_takeover = uniroot(price_equilibrium, interval = c(0,500000))$root

# Compute farmed output from FOC of equation sup.mat 41
q_farmed = (alpha_f - 2*gamma*q_takeover - v)/(2*beta_f)

profit_takeover = (alpha_w - beta_w*q_takeover - gamma*q_farmed - 
                     (price_poacher_takeover +c))* q_takeover +
  (alpha_f - beta_f*q_farmed - gamma*q_takeover - v)*q_farmed - 
  (q_farmed %/% 144 + 1)*cost_pen
profit_takeover_mil = profit_takeover/10^6

# 2. Data for comparison with scenarios
q_monop = growth(x_monop)
price_monopoly = uniroot(function(s) price_equilibrium(s, market_structure = 'monopoly'), interval = c(0,5000000))$root
#price_poachers = (W2 * (alpha-c) + beta * W1 * sigma * x_monop)/(beta * sigma^2 * x_monop^2 + W2)
profit_monopoly = (alpha - beta * q_monop - (price_monopoly + c)) * q_monop
profit_monopoly_mil = profit_monopoly/10^6

q_cournot = growth(x_cournot)
price_cournot = uniroot(function(s) price_equilibrium(s, market_structure = "cournot"), interval = c(0,500000))$root
q_farmed =  (2*beta_w*(alpha_f - v) - gamma*(alpha_w - price_cournot - c))/(4*beta_w*beta_f - gamma^2)
profit_cartel = (alpha_w - beta_w*q_cournot - gamma*q_farmed - (price_cournot + c))*q_cournot
profit_cartel_mil = profit_cartel / 10^6

# 3. Reduced bioecon perf
takeover_performance = data.frame(
  Scenario = c("Vertical Monopoly",
               "Cournot Competition",
               "Cartel takeover"),  
  harvest = c(q_monop, q_cournot, q_takeover),
  stock = c(x_monop, x_cournot, x_takeover),
  price = c(price_monopoly, price_cournot, price_poacher_takeover),
  Profit = c(profit_monopoly_mil, profit_cartel_mil, profit_takeover_mil)
)
takeover_performance %>%
  rename("Harvest in (mt)"= harvest,
         "Stock in (mt)" = stock, 
         "Price paid to poachers (USD/mt)" = price, 
         "Profit in 10^6 USD" = Profit) %>% 
  tt(caption = 'Bioeconomic performance of takeover scenario')
```
...
```
The observed price per kilogram of totoaba in most recent years is `r mean_price_ton_toto_recent/1000` USD. The model is expressed in metric tons, so we use the metric ton numbers instead to parameterize an idea of the worst case trading costs.
