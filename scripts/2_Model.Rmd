...
-   `monop_harvest(x, ...), monop_harvest_lq(x, ...)`: harvest (in metric tons) when trader is a monopolist in the quadratic and linear quadratic cost models.
...
-   `cournot_harvest(x, ...)`, `bertrand_harvest(x, ...), cournot_harvest_lq(x, ...)`, `bertrand_harvest_lq(x, ...)`: harvest (in metric tons) when trader and farmer compete in *Cournot*, i.e, set quantities strategically, and *Bertrand*, i.e, set prices strategically in the quadratic and linear quadratic cost models. 
...
plot_ = for_graph %>%
  ...
  scale_color_manual(values = c('black','forestgreen','#1f78b4', 'firebrick3') )
