...
p1 = dat_%>%
  subset(x %% 2 == 0)%>%
  subset(quantile %in% unique(dat_$quantile)[c(1,2)])%>%
  ggplot(aes(x = x,
             y = values2, 
             color = new_names))+
...
p2 = dat_%>%
  subset(x %% 2 == 0)%>%
  subset(quantile %in% unique(dat_$quantile)[c(8,9)])%>%
  ggplot(aes(x = x,
             y = values2, 
             color = new_names))+
  geom_line(linewidth = 1)+
...
if(!file.exists(here('visuals', 'supplementary_visuals', 'equilibria_with_W1_W2.png'))){
  final
  ggsave(here('visuals', 'supplementary_visuals', 'equilibria_with_W1_W2.png'),
        width = 30,
        height = 15, 
        units = 'cm') 
}
...
dat_%>%
  ...
  theme(legend.position = 'bottom',
        ...
        legend.text = element_text(family = font_), 
        legend.title = element_text(family = font_))
ggsave(here('visuals','figure2.png'), height = 12, width = 20, units = 'cm')
```
...
bioecon_table_large2 %>%
  tt()%>%
  style_tt(i =0 , j=1:7, background = '#EEDD72')%>%
  style_tt(i = 1, j= 1:7, background = '#EEDD62')%>%
  style_tt(i = 8, j = 1:7, background = '#EEDD62')%>%
  style_tt(i = c(2,3,6,7,10,11), j = 1:7, background = '#EEDD82')%>%
