...
a = storage_stable %>%
  filter(W < 1.01*W_baseline & W>.999*W_baseline)%>%
  filter(W1 < 1.01* W1_baseline & W1 > .999*W1_baseline)%>%
  filter(W2 < 1.01 *W2_baseline & W2_baseline >.999 * W2_baseline)%>%
...
z = z[order(z$Scenario, decreasing= T),]
new_names_z = c()
for(x in 1:nrow(z)){
  to_glue = strsplit(z[x, 'Scenario'], ' ')
  to_glue =paste(to_glue[[1]][1], to_glue[[1]][2])
  new_names_z = append(new_names_z, to_glue)
}

z$Scenario = new_names_z


write.csv(z, here('data','outputs', 'bioecon_performance_table.csv'))

z %>%
  tt(caption = 'Bioeconomic performance')%>%
  style_tt(i = c(0,2), background = '#EEDD82')
