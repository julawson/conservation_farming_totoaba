...
result1 %>%
  select(x, q_bertrand_harvest, q_bertrand_harvest_illustration, monop_harvest)%>%
  pivot_longer(cols = -c('x'), 
               names_to = 'names',
               values_to = 'values')%>%
  ggplot(aes(x = x, y = values, colour = names, linetype = factor(names)))+
  geom_line()+
  scale_color_manual(values = c( 'mediumseagreen','mediumseagreen','forestgreen'),
                     labels = c('Price setting quantity', 'Price setting quantity with lower v (dotted)', 'Vertical monopoly'))+
...
## Figure X : Harvest functions for monopoly v. price taker

## Figure X : Bertrand v. Monopoly following Damania and Bulte

## Figure X : Comparison between monopoly and Bertrand behavior on monopolistic and duopolistic markets following Damania and Bulte

## Figure X : F (x) and v summarise when Bertrand harvest is larger or lower than in monopoly.