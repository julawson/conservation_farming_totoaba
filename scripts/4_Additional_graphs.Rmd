...
dat_b %>% 
  ...
  labs(color=NULL)
    ggsave(here("visuals/supplementary_visuals/", "fig1_trends_stock_catch.png"))
```




## Figure 2 (SM) and 1 in Main : Hypothetical schematic of equilibrium points under different poaching harvest functions
...
data.frame(x = seq(1,100))%>%
  ...
  ggsave(here("visuals", "Figure2_potential_equilibria.jpg"))
```
...
plot_ = result1 %>%
  ...
  theme(legend.position = 'bottom',
        ...
        axis.title.y  = element_text(family = font_),
        legend.text = element_text(family = font_), 
        legend.title = element_text(family = font_))plot_
ggsave(here('visuals', 'supplementary_visuals', 'Figure3c.jpg'))


```

## Figure 4 : Bertrand v. Monopoly following Damania and Bulte
```{r, fig 4}

results %>% 
  subset(!(names %in% c("Bertrand wild harvest (own)")))%>%
  ggplot(aes(x = x, 
             y = values, 
             color = names))+
  geom_line()+
...
## Figure 5 : Comparison between monopoly and Bertrand behavior on monopolistic and duopolistic markets following Damania and Bulte

```{r, fig 5}
lemma = data.frame(x, # Growth
                   "Monopoly on a duopolistic market" = harvest_monop_direct(a_w, b_w, c, W, sigma_min, x), # Monopoly using the duopolistic market assumption
                   "Monopoly on a monopolistic market" = harvest_monop_direct(a_m, b_m, c, W, sigma_min, x), # Monopoly using the monopolistic market assumption
                   "Bertrand on a duopolistic market" = q_b_paper(a_f, a_w, b_f, b_w, c, e, v, W, sigma_min, x)) %>% # Bertrand harvest using the article's formula
  pivot_longer(cols = -c('x'),
               values_to = 'values',
               names_to = 'names')

colors = c('royalblue1','chartreuse','forestgreen' )
# Plot
...
lemma %>% 
  ...
  scale_color_manual(values = colors, 
                     labels = c("Bertrand on a duopolistic market",
                                "Monopoly on a duopolistic market",
                                "Monopoly on a monopolistic market"
                                ))+
  theme_bw()+
  xlab('Population ')+
  ylab("Harvest")+
...
## Figure 6 : F (x) and v summarise when Bertrand harvest is larger or lower than in monopoly
...
tester %>%
  ...
  geom_line(aes(y = v_), linewidth = 1.1, linetype = 'dotted')+
  ...
  geom_ribbon(data=subset(tester,  x>5000 & x <= 9000), 
              aes(ymin=v,ymax=v_func), fill="lightgreen", alpha=0.5) +
  geom_ribbon(data=subset(tester, x>=9000), 
...
## Figure 7 : v for poaching to stop in a Bertrand equilibrium
```{r, fig7}
...
## Figure 8 : Price per ton of totoaba and catch from 2014 to 2017
```{r, fig 8}
...
ggsave(here('visuals/supplementary_visuals', 'Figure8_price_data.jpg'))

```

## Figure 9 : Von Bertlaffery Growth curves
```{r}
...
ggsave(here("visuals", 'supplementary_visuals', 'Figure9_VBGF_growth.jpg'))
