...
### 1. Latest attempt for costs estimation 
...
which(least_squares$value == min(least_squares$value))


#lm(total_cost ~ trip + squared_trip, costs2)

 costs2 %>%
  mutate(estimated_linear_quadratic = (costs[2]+costs[3])/1.75 * trip + .75* squared_trip)%>%
...
* `r c(W1_potential[3], W2_potential[3])` : similar, but drastically closer to the actual observed cost after the jump, and close to the marginal cost at observed historical values. 
...
Recover costs for the linear quadratic model :
...
write.csv(W_lq, here('data','inputs','W_lq.csv'), row.names = F)
```
