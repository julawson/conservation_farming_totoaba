...
if(!(all(c('Fishlife') %in% installed.packages()))){
...
First, as we have time series, seasonal data, we can use it to reconstruct an average cost per sub-season.
...
W1_potential = c((costs[2]+costs[3])/2, (costs[2]+costs[3])/1.9, (costs[2]+costs[3])/1.75, (costs[2]+costs[3])/1.6, costs[3])
W2_potential = c(1.6, 1.3, .75,.1, 0)
W_lq_new = data.frame(W1_potential, W2_potential)%>%
  mutate(AC = costs[3], 
 ...
 costs_final%>%
   ...
   geom_line()+
   geom_vline(xintercept = max_E)
   
```

I think this is a more convincing estimate of the cost function. In order to do a sensitivity analysis we can do : 

With these values, we get different flavors : 

* `r c(W1_potential[1], W2_potential[1])`: first overestimates the costs a little bit before the discontinuity, underestimates them later on, but matches closely the constant marginal cost

* `r c(W1_potential[2], W2_potential[2])` : similar but closer

* `r c(W1_potential[3], W2_potential[3])` : similar, but drastically closer to the actual observed cost after the jump, and close to the marginal cost at observed historical values  - **this is the one we use in the graph and in the baseline specification**

* `r c(W1_potential[4], W2_potential[4])` : very close to curve after jump, but overestimates costs early on a lot 

* `r c(W1_potential[5], W2_potential[5])` : matches final part of the curve and has no square. 
...
params = c('alpha', 'beta', 'r', 'k', 'sigma', 'W_mid', 'W_low', 'W_high', 'avg_cost','W1', 'W2', 'age', 'gamma', 'v', 'c')
value = c(alpha, beta, r, k, catchability, W_qmid, W_qlow, W_qhigh, W_high, 
          W_lq_new[baseline_W_choice,'W1_potential'], W_lq_new[baseline_W_choice, 'W2_potential'], age, gamma, v, c)
concept = c('Demand model : intercept', 
            ...
            'Parameter cost (MC = Avg) at historical value - high season', 
...
units = c('USD', 'USD/metric ton of biomass', 'unitless',   'metric tons of biomass','% of biomass/vessel trip', 'USD vessel trip$^{-2}$',  'USD vessel trip$^{-2}$', 'USDvessel trip$^{-2}$', 'USD/vessel trip', 'USD/vessel trip', 'USD vessel trip$^{-2}$', 'Years', 'Unitless', 'USD/metric ton of biomass', 'USD/ metric ton of biomass')
