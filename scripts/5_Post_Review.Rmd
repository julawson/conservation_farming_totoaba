...
Now ewploit the data with the new cost structure : 
...
final_dat = data.frame(0,0,0,0,0,0,0,0)
colnames(final_dat) = colnames(storage)

for(name in unique(storage$names)){
  study = storage %>%
    subset(names == name)%>%
    mutate(lag_sign = c(0, diff(sign(values))),
           lag_val = c(0, diff(x)))
...
rename_scenario = function(x){
  ...
  }else if(x =='ss_bertrand'){
    return('Price sett. Quad.')
  }else if(x =='ss_bertrand_lq'){
    return('Price sett. Lin. Quad.')
  }else if(x == 'ss_cournot'){
    return('Qty Quad.')
...
final_dat %>%
  ...
  facet_grid(new_AC~new_quantile)+
  theme_bw()+
  ggtitle("Quantity setting steady states with linear and linear quadratic costs")+
  theme(axis.text.x = element_text(angle = 60, hjust = 1), 
        axis.title.x= element_blank(),
        axis.title.y = element_blank(),
        plot.title = element_text(hjust = 0.5))
...
Need to put the data back in wide, to recompute the rate of variation and back to long to plot. 
...
# Loop over costs
for(cost in unique(storage$AC)){
  working_data = storage %>%
    subset(AC == cost)
  # Loop over quantiles
  for(quant in unique(storage$quantile)){
    working_data = working_data %>%
      subset(quantile == quant)%>%
      subset(values<.05 & values >-.05)%>%
      arrange(names)
    #L Loop over each type of equilibrium
    for(name in unique(working_data$names)){
      working_data2 = working_data %>% subset(names==name)
      if(nrow(working_data2)>1){
        # If there is more than 1 steady state :
        working_data2 = working_data2 %>% 
          mutate(diff_sign = c(0,diff(sign(values))), # variable to find different steady state zones
                 diff_x = c(0,diff(x))) # variable to locate the changing of sign
        
        cuts_ = which(working_data2$diff_x>10) # Define cut variable to study only the right steady state zones
        
        if(cuts_[1]!=1){
          cuts_ = c(1,cuts_)
        }
        
        if(cuts_[length(cuts_)] == nrow(working_data2)){
          cuts_ = c(cuts_,(nrow(working_data2)+1))
        }
        
        for(cut in 1:(length(cuts_)-1)){
          study = working_data2[cuts_[cut] : cuts_[cut+1]-1, ]%>%
            subset(diff_sign !=0)%>%
            select(-c(diff_sign, diff_x))
          final_dat = rbind(final_dat, study)
        }
        
        to_keep = which(diff(sign(working_data2$values))!=0)
        working_data2 = working_data2[to_keep,]
        
      }else{
      final_dat = rbind(final_dat, working_data2)
      }
    }
  }
}
