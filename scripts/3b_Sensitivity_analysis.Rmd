...
store %>%
  ...
  geom_point(data = data.frame(x = baselines_ %>% select(v) %>% pull() %>% unique(), 
                               y = baseline_bertrand, 
<<<<<<< HEAD
                               names = 'Price setting'),aes(x = x, y = y), colour = '#B22222', size = 2.5)+
    geom_point(data = data.frame(x = baselines_ %>% select(v) %>% pull() %>% unique(), 
                               y = baseline_cournot, 
                               names = 'Quantity adjustment'),aes(x = x, y = y), colour = '#1f78b4', size = 2.5)+
=======
                               names = 'Price setting'),aes(x = x, y = y), colour = '#B22222')+
    geom_point(data = data.frame(x = baselines_ %>% select(v) %>% pull() %>% unique(), 
                               y = baseline_cournot, 
                               names = 'Quantity adjustment'),aes(x = x, y = y), colour = '#1f78b4')+
>>>>>>> c387a7f635befaf3aff5e5c0e2e6160fcb0a19ee
  facet_grid(~names)+
...
z = model_illustration(params_sensitivity[12946, ])
