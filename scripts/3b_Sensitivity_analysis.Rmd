...
### G. Subsidy on demand 
v = baseline$v
v_sens = v* c(1, 0.9, .8, .7, .6)

sensitivity = do.call(rbind, replicate(length(v_sens), baseline, simplify = F))%>%
  mutate(v = v_sens,
         spec = 'subsidy',
         transformation = c(1, 0.9, .8, .7, .6))
params_sensitivity = rbind(params_sensitivity, 
                           sensitivity)
...
if(!(file.exists(here('data','outputs','sensitivity_analysis_results.csv')))){
  write.csv(exploit_, here('data', 'outputs', 'sensitivity_analysis_results.csv'))
}
```
...
cost_sens %>%
  mutate(multiplier_global = as.numeric(multiplier_global),
         ...
         W2_multiplier = as.numeric(W2_multiplier),
         namer = paste(round(W1,1), " and ", round(W2,1)),
         baseline = as.factor(ifelse(W1_multiplier ==1 & W2_multiplier == 1, 1, 0)),
         x_norm = (x - baseline_stock)/baseline_stock*100)%>%
  subset(Stable == "Stable")%>%
  filter(multiplier_global == 1)%>%
  ggplot(aes(x = namer, y =x_norm, color = names, shape = names))+
  geom_point(size = 2.5)+
  scale_color_manual(values = c('#B22222','#1f78b4', '#248721'))+
...
v_sens %>%
  mutate(transformation = paste((1 - as.numeric(transformation))*100, '%'),
         x_norm = (x - baseline_stock)/baseline_stock * 100)%>%
...
```
### 3. Impact of different substitutability
...
### 3. Impact of joint variation of demand parameters
