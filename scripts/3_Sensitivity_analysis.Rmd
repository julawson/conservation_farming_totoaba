...
df_cv1 %>%
  ...
  labs(color = 'Cost of transaction (in USD)')



ggsave(here('visuals', 'Figure5b.jpg'))
#  geom_point(data = new_c, col = 'grey')
#  geom_point(data = new_v, col = 'blue')
```
...
checker = store %>%
  ...
  mutate(`Cournot normalized` = paste0(round((ss_cournot1/ss_baseline - 1)*100,2), '%'),
         `Bertrand normalized` = paste0(round((ss_bertrand1/ss_baseline -1)*100,2), '%'))%>%
...
pal = c("#febd2a", "#f48849", '#db5c68', '#b83289','#8b0aa5', '#9049EE', '#4580CD')
checker %>%
  ...
  drop_na()%>%
  ggplot(aes(x = names, y=values, colour = as.factor(round(alpha,2))))+
  geom_point(size = 3)+
  ...
  ylab(' ')+
  theme(legend.position= 'bottom')+
  labs(colour = 'Demand multiplier')
ggsave(here('visuals/supplementary_visuals', 'sens_alpha_gamma.jpg'))
...
colnames(storage) = c('gamma', 'Monopoly', 'Cournot', 'Bertrand', 'alpha', 'Equilibrium')
thresh = c(0, 2450, 10000, 500000)
for(x in c(1, 1.2, 1.4, 1.6)){
  for( i in seq(1, length(thresh)-1)){
    a = checker %>%
    subset(as.character(alpha) ==x)%>%
    filter(values > thresh[i] & values < thresh[i+1])%>%
    select(-c(values, alpha))%>%
    drop_na()%>%
    pivot_wider(names_from=names, values_from = values2)%>%
...
if(Latex == T){
  storage = as.data.frame(storage) %>%
    ...
    mutate(gamma = paste0(gamma, '\ %'),
           Monopoly = round(unlist(Monopoly), 2), 
           Cournot = round(unlist(Cournot), 2), 
           Bertrand = round(unlist(Bertrand), 2))
