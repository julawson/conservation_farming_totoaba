...
### C. Trading costs
c_ = seq(0, 350000, by = (350000/7))
sensitivity = do.call(rbind, replicate(length(c_), baseline, simplify = F))%>%
  mutate(c = c_,
         spec = 'c',
         transformation = c_)
params_sensitivity = rbind(params_sensitivity, 
...
c_sens_ = seq(0, 350000, by = round(350000/7))

to_append = expand.grid(v_sens,
                        c_sens_)

sensitivity = do.call(rbind, 
                      replicate(nrow(to_append), baseline, simplify = F)
                      )%>%
  mutate(v = to_append$Var1,
         c = to_append$Var2,
         spec = 'v and c',
         transformation = paste('c=',round(to_append$Var1/c_sens_,2),'and v=',round(to_append$Var2/v_sens,2)))
params_sensitivity = rbind(params_sensitivity, 
                           sensitivity)
### B. c and v and gamma
...
sensitivity = do.call(rbind, replicate(nrow(to_append), baseline, simplify = F))%>%
  mutate(v = to_append$Var1,
         ...
         transformation = paste('c=',round(to_append$Var1/c_sens_,2),'and v=',round(to_append$Var2/v_sens,2), 'and gamma', to_append$Var3/gamma_sens))
params_sensitivity = rbind(params_sensitivity, 
                           sensitivity)
...
if(!(file.exists(here('data','inputs', 'params_sensitivity.csv')))){
  write.csv(params_sensitivity, here('data', 'inputs', 'sensitivity_analysis.csv'))
}
```
...
if(!(file.exists(here('data','outputs','sensitivity_analysis.csv')))){
  ...
  if(nrow(params_sensitivity) > 10000){
    ...
    pb <- txtProgressBar(min = 0, max = nrow(params_sensitivity), style = 3)
...
store %>%
  subset(names != 0)%>%
  mutate(c = factor(c, levels = seq(0,350000, 50000)), )%>%
  ...
  geom_point(data = data.frame(x = baselines_ %>% select(v) %>% pull() %>% unique(), 
             ...
             colour = '#B22222',
             size = 2.5)+
    geom_point(data = data.frame(x = baselines_ %>% 
                                   select(v) %>%
                                   pull() %>% 
                                   unique(), 
                               y = baseline_cournot, 
                               names = 'Quantity adjustment'),
               aes(x = x, y = y), 
               colour = '#1f78b4',
               size = 2.5)+
  facet_grid(~names)+
...
ggsave(here('visuals', 'sup_figure3.png'), height = 22.5, width = 15, units = 'cm')
...
theme(axis.text.x = element_text(angle = 60,
        ...
        aspect.ratio = 10/4)
p
ggsave(here('visuals', 'Figure5.jpg'), p, width = 20, height = 15, units = 'cm')
```

```{r, support for numbers in last paragraph}
graph_dat%>%
  subset(gamma3 %in% c('50%'))%>%
  select(c(identifier,names, x, alpha, gamma3, Stable))%>%
  subset(alpha %in% c(1.8))
